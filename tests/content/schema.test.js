...
function validCriterion(overrides = {}) {
  return {
    ...
    codeExample: { lang: 'css', bad: 'color: #999 on #fff;', good: 'color: #595959 on #fff;' },
    howToTest: 'Use a contrast checker on text/background color pairs.',
    check: { question: 'What ratio is required for normal text?', choices: ['3:1', '4.5:1', '7:1'], answer: 1 },
    references: [],
    ...overrides,
  };
}
...
describe('criterionSchema', () => {
  ...
  it('rejects an id that does not match the SC number pattern', () => {
    expect(() => criterionSchema.parse(validCriterion({ id: 'not-an-id' }))).toThrow();
  });
  ...
  it('rejects a principle outside the four WCAG principles', () => {
    expect(() => criterionSchema.parse(validCriterion({ principle: 'Nonsense' }))).toThrow();
  });

  it('rejects fewer than 2 answer choices', () => {
    expect(() =>
      criterionSchema.parse(validCriterion({ check: { question: 'Q?', choices: ['only one'], answer: 0 } }))
    ).toThrow();
  });
  ...
  it('rejects an answer index out of range for the given choices', () => {
    expect(() =>
      criterionSchema.parse(
        validCriterion({ check: { question: 'Q?', choices: ['a', 'b'], answer: 5 } })
      )
    ).toThrow();
  });
